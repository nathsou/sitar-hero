...
A four-lane classical rhythm game dressed as an eighteenth-century music salon. Choose a piece, then press **D**, **F**, **J**, or **K** when its approaching jeweled notes reach the gilded rings. For a ribbon note, keep the key down until its tail reaches the ring; releasing early breaks the hold and fades the melody. The on-screen keys and track also work with a pointer or touch.

In the programme, **D/F** choose a piece or adjust the highlighted slider, **J/K** move between the piece list, difficulty, speed, and start button, and **Space** starts immediately. Previews are on by default: the selected piece plays automatically, crossfades to the next selection, and loops until you begin. **L** toggles previews. Browser autoplay rules may require the first key press or click before sound begins. During play, **Space** pauses. In the pause menu, **J** resumes and **K** returns to the programme. On the results screen, **J** replays and **K** returns to the programme. **O** opens the options menu from any screen; opening it during play pauses the piece. There, **D/F** toggle the lane key hints shown beneath the notes and **O/Escape** closes the menu. Hiding those hints keeps the track playable by keyboard or touch. **Shift+F** toggles full screen; the visible buttons, Tab, and standard slider arrow keys also work.
...
Perfect, great, and good hits earn 1,000, 650, and 350 points, plus a streak bonus. Completed holds add points for their duration; an early release counts as a miss. The result screen shows your accuracy and best streak. Personal bests are saved in local storage for each piece, difficulty, and speed setting, and appear in the programme when you return.
